...
.info-bar { display: flex; flex-wrap: wrap; gap: 6px 18px; background: #f8faff; border: 1px solid #dbeafe; border-radius: 9px; padding: 10px 14px; margin-bottom: 1.75rem; font-size: 12px; color: #6b7280; }
...
.p-row { display: grid; grid-template-columns: 36px 110px 1fr; gap: 8px; padding: 7px 13px; border-bottom: 1px solid #f3f4f6; font-size: 12px; align-items: start; }
...
<div class="card">
  ...
  <table class="cat-table">
    <thead>
      <tr><th>Category</th><th>HS Code</th><th>Part B 상단 텍스트</th></tr>
    </thead>
    <tbody>
  ...
  <textarea id="inp" placeholder="엑셀에서 NO. / Category / Description 세 열을 복사해서 붙여넣으세요.
...
function parse() {
  ...
  lines.forEach((l, i) => {
    ...
    if (parts.length >= 3) {
      ...
      const validCats = ['CHECKING FIXTURE','PRESS TOOL','JIG'];
      if (/^\\d+$/.test(no) && validCats.includes(cat) && desc) {
  ...
  list.innerHTML = ok.map(it =>
    `<div class="p-row"><span class="p-no">${it.no}</span><span class="p-cat">${CAT_BADGE[it.cat]||it.cat}</span><span class="p-desc">${xesc(it.desc)}</span></div>`
  ).join('') + (bad.length ? `<div class="p-row err-row"><span class="p-err">⚠ ${bad.length}줄 인식 실패 — Category가 정확한지 확인하세요</span></div>` : '');
...
function fname(no, cat) {
  const ext = fmt === 'pdf' ? 'pdf' : 'docx';
  return `FormI_HS2026-05-302P_NO ${String(no).padStart(3,'0')}.${ext}`;
}
...
async function genOne() {
  ...
  try {
    for (let i = 0; i < items.length; i++) {
      ...
      const a = document.createElement('a'); a.href = url; a.download = fname(no, cat); a.click();
